                ...
                if crc == crc_rx:
                    ...
                    if id1 == self.ID1["SETUP_GSM_RESP"]:
                        ...
                        if self.status_new == self.ID2["SIM900_USER_TIMEOUT_READ"] and id2 == self.ID2["SIM900_USER_TIMEOUT_READ"]\
                            ...
                            # выводим полученные данные в окно вывода
                            self.user_bit_num = self.ui.list_users_nom.currentRow() #Convert_To_User_Num(self.ui.list_users_nom.currentRow())
    ...
    def timerEvent(self, e):
        ...
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                ...
                if self.result_analyze == ['Ok']:
                    ...
                    # выводим полученные данные в окно вывода
                    self.ui.lineEdit_in_alarm_text.setText('')
                    QtWidgets.QMessageBox.warning(self, 'Ошибка', 'Текст СМС о срабатывании не получен',
                                                  QtWidgets.QMessageBox.Ok)
        ...
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            if self.result_analyze != ['Ok']:
                # выводим пустые данные в окно вывода
                self.ui.spinBox_sim900_user_timeout.setValue(0)
                QtWidgets.QMessageBox.warning(self, 'Ошибка', 'Длительность входящего разговора по GSM не получена',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                # выводим пустые данные в окно вывода
                self.ui.lineEdit_Telephone_Number.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка', 'Телефонный номер не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                # выводим пустые данные в окно вывода
                self.ui.checkBox_call_back_alarm.setChecked(0)
                QtWidgets.QMessageBox.warning(self, 'Ошибка', 'Флаг разрешения/запрещения звонка при авариине не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                # выводим пустые данные в окно вывода
                self.ui.checkBox_SMS_alarm.setChecked(0)
                QtWidgets.QMessageBox.warning(self, 'Ошибка', 'флаг разрешения/запрещения отправки СМС оповещения при аварии не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                ...
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'флаг разрешения/запрещения отправки СМС оповещения о постановке на охрану не получен',
                                              QtWidgets.QMessageBox.Ok)
        ...
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            if self.result_analyze != ['Ok']:
                # выводим пустые данные в окно вывода
                self.ui.checkBox_call_back_alarm.setChecked(0)
                QtWidgets.QMessageBox.warning(self, 'Ошибка', 'Флаги разрешения/запрещения звонка при авариине на получены',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                # выводим пустые данные в окно вывода
                self.ui.checkBox_SMS_alarm.setChecked(0)
                QtWidgets.QMessageBox.warning(self, 'Ошибка', 'флаги разрешения/запрещения отправки СМС оповещения при аварии не получены',
                                              QtWidgets.QMessageBox.Ok)
        ...
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            if self.result_analyze != ['Ok']:
                # выводим пустые данные в окно вывода
                self.ui.lineEdit_text_guard_set.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Текст СМС об успешной установке на охрану не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                # выводим пустые данные в окно вывода
                self.ui.lineEdit_clear_alarm_text.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Текст СМС для снятия аварии не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                # выводим пустые данные в окно вывода
                self.ui.lineEdit_sms_reset_guard_cmd.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Текст СМС для снятия с охраны не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                # выводим пустые данные в окно вывода
                self.ui.lineEdit_sms_Set_Guard_Cmd.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Текст СМС для установки на охрану  не получен',
                                              QtWidgets.QMessageBox.Ok)
        ...
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ...
            if self.result_analyze != ['Ok']:
                ...
                self.ui.lineEdit_sms_Set_Guard_Cmd.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Минимальный остаток на счету для СМС не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                ...
                self.ui.lineEdit_sms_Set_Guard_Cmd.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Минимальный остаток на счету для звонков не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                ...
                self.ui.lineEdit_sms_Set_Guard_Cmd.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Текст донабора DTMF для сброса аварии не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                ...
                self.ui.lineEdit_sms_Set_Guard_Cmd.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Текст донабора DTMF для снятия с охраны не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                ...
                self.ui.lineEdit_sms_Set_Guard_Cmd.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Текст донабора DTMF для установки на охрану не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                ...
                self.ui.lineEdit_sms_Set_Guard_Cmd.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Конец команды «Перезвоните мне» не получен',
                                              QtWidgets.QMessageBox.Ok)
            ...
            if self.result_analyze != ['Ok']:
                ...
                self.ui.lineEdit_sms_Set_Guard_Cmd.setText('')
                QtWidgets.QMessageBox.warning(self, 'Ошибка',
                                              'Начало команды «Перезвоните мне» не получено',
                                              QtWidgets.QMessageBox.Ok)
